...
export default function EditorPanel({ markdownInput, onInputChange, editorScrollRef, onEditorScroll, scrollSyncEnabled, onSelectAll, onClearRequest, onAbortStream, onRegenerateStream, isThinking: thinking, immersive }: EditorPanelProps) {
    const [copied, setCopied] = useState(false);

    const handleCopy = useCallback(async () => {
        if (copied || !markdownInput) return;
        try {
    ...
    }, [copied, markdownInput]);

    const onPaste = (e: React.ClipboardEvent<HTMLTextAreaElement>) => {
        handleSmartPaste(e, onInputChange);
    };
    const isThinking = Boolean(thinking);
    ...
    return (
        <div className="border-r border-[#00000015] dark:border-[#ffffff15] flex flex-col relative z-30 bg-transparent flex-1 min-h-0">
            ...
            {immersive ? (
                <div className="absolute bottom-3 right-3 flex items-center gap-1.5 px-2 py-1 rounded-md bg-white/40 dark:bg-black/40 backdrop-blur-md shadow-sm">
